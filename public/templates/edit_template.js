...
async function loadTemplateFromServer() {
  try {
    ...
    if (data.status === "success") {
      ...
      alert(
        "No template selected or session expired. Redirecting to dashboard.",
      );
...
function updateIframeContent() {
  ...
  const html_code =
    htmlInput.value.trim() || "<p>Start typing your HTML template.</p>";
...
orientationInputs.forEach((r) =>
  r.addEventListener("change", applyOrientation),
);
...
form.addEventListener("submit", async (e) => {
  ...
  if (!templateId) {
    alert(
      "Template ID missing. Please select a template from the dashboard first.",
    );
    window.location.href = "../dashboard/dashboard.html";
    return;
  }
